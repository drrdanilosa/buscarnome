...
export class ForumCrawler {
  ...
  async searchForum(forumUrl, username, options = {}) {
    if (!this.enabled) {
      logger.warn('Forum search attempted while disabled', { forumUrl, username });
    ...
    logger.info(`Searching forum ${baseUrl} for username: ${username}`);
    
    try {
      // Emit event for search start
      this.eventBus.emit('forumCrawler:searching', { forumUrl: baseUrl, username });
