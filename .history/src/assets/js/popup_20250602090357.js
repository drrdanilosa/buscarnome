...
async function startSearch(username, options) {
  ...
  try {
    ...
    // Enviar solicitação de busca com tratamento específico para Firefox
...
async function checkSearchStatus() {
  ...
  try {
    // Verificar se a API está disponível
    if (!browserAPI || !browserAPI.runtime) {
      console.error('[DeepAlias Popup] API não disponível para verificar status');
      return;
    }
    ...
    const response = await new Promise((resolve, reject) => {
      browserAPI.runtime.sendMessage({
        ...
        if (browserAPI.runtime.lastError) {
          console.error('[DeepAlias Popup] Erro ao obter status:', browserAPI.runtime.lastError);
          reject(new Error(browserAPI.runtime.lastError.message));
        } else {
          resolve(response);
        }
