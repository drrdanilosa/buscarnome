...
async function initialize() {
  ...
  // Diagnóstico Firefox
  const diagnosticoFirefox = document.getElementById('diagnostico-firefox');
...
/**
 * Executa diagnóstico completo da extensão no Firefox
 * @param {Event} event - Evento de clique
 */
async function executarDiagnosticoFirefox(event) {
  ...
  // Evento para fechar
  content.querySelector('#fechar-diagnostico').addEventListener('click', () => {
  ...
  try {
    ...
    try {
      ...
      if (pingResponse) {
        ...
        addResult(`📥 Resposta: ${JSON.stringify(pingResponse)}`, 'info');
      } else {
        addResult('⚠️ Background não respondeu', 'warning');
      }
    } catch (error) {
    ...
    try {
      ...
      if (platformsResponse && platformsResponse.platforms) {
        const count = platformsResponse.platforms.length;
        if (count > 0) {
          addResult(`✅ ${count} plataformas carregadas`, 'success');
          addResult(`📋 Exemplos: ${platformsResponse.platforms.slice(0, 3).map(p => p.name || p).join(', ')}`, 'info');
        } else {
    ...
    try {
      ...
      addResult(`⚠️ Storage pode ter problemas: ${error.message}`, 'warning');
    }
