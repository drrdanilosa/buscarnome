...
async function startSearch(username, options) {
  ...
  try {
    ...
    if (response.success) {
      ...
      handleSearchError(response.error || 'Erro desconhecido ao iniciar busca');
    }
  } catch (error) {
...
/**
 * Verifica o status da busca atual
 */
...
function openHelpPage(event) {
  event.preventDefault();
  browser.tabs.create({ url: browser.runtime.getURL('src/help/help.html') });
}
...
function openAboutPage(event) {
  event.preventDefault();
  browser.tabs.create({ url: browser.runtime.getURL('src/about/about.html') });
}
