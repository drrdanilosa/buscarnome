...
const verificacoes = {
    background: [
        { buscar: 'typeof browser !== \'undefined\' && browser.runtime', desc: 'Detecção Firefox' },
        { buscar: 'typeof chrome !== \'undefined\' && chrome.runtime', desc: 'Detecção Chrome' },
        ...
        { buscar: 'Instagram.*Twitter.*TikTok.*YouTube', desc: 'Plataformas principais definidas' }
    ],
    popup: [
        { buscar: 'function sendMessageToBackground', desc: 'Função auxiliar de comunicação' },
        { buscar: 'isFirefox.*typeof browser', desc: 'Detecção Firefox no popup' },
        { buscar: 'isChrome.*typeof chrome', desc: 'Detecção Chrome no popup' },
        { buscar: 'loadSettings.*sendMessageToBackground', desc: 'loadSettings usando função auxiliar' },
        { buscar: 'startSearch.*sendMessageToBackground', desc: 'startSearch usando comunicação específica' },
        { buscar: 'executarDiagnosticoFirefox', desc: 'Diagnóstico Firefox implementado' }
    ],
    content: [
        { buscar: 'typeof window.fetch === \'function\'', desc: 'Verificação de fetch' },
        { buscar: 'descriptor.configurable', desc: 'Verificação de configurabilidade' },
        { buscar: 'originalFetch.apply', desc: 'Interceptação de fetch' }
    ],
    adapter: [
        { buscar: 'typeof browser !== \'undefined\'', desc: 'Detecção segura Firefox' },
        { buscar: 'typeof chrome !== \'undefined\'', desc: 'Detecção segura Chrome' },
        { buscar: 'Runtime API não disponível', desc: 'Tratamento de erro' }
    ],
    popupHtml: [
    ...
    manifest: [
        ...
        { buscar: '"background".*"scripts"', desc: 'Background scripts configurados' }
    ]
};
...
function verificarArquivo(nomeArquivo, caminhoArquivo, verificacoesArquivo) {
    ...
    for (const verificacao of verificacoesArquivo) {
        totalVerificacoes++;
        const regex = new RegExp(verificacao.buscar.replace(/\./g, '[\\s\\S]*'), 'i');
